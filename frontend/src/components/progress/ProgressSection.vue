  ...
  <div class="section-block">
    ...
    <template v-else-if="editing || adding">
      ...
      <div class="flex gap-8 mt-8" style="justify-content:flex-end">
        ...
        <button class="btn btn-primary btn-sm" @click="handleSave" :disabled="!hasContent(issueText)">저장</button>
      </div>
    </template>
...
async function handleSave() {
  if (!hasContent(issueText.value)) return
  try {
    const payload = {
      week: props.week,
      ...
      assignee: props.progress?.assignee || '',
    }
    let saved
    if (props.progress) {
      const { data } = await axios.put(`/api/progress/${props.progress.id}`, { issue: issueText.value.trim() })
      saved = data
    } else {
