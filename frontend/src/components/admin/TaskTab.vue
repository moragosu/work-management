<template>
              ...
              <tr :id="'task-row-' + t.id" :class="{ 'row-selected': selectedTaskIds.includes(t.id) }">
                ...
                <td>
                  <div class="flex gap-8">
                    <button class="btn btn-ghost btn-xs" @click="router.push(`/tasks/${t.id}/history?back=/admin%3Ftab%3Dtask`)" data-tooltip="주차별 이력 보기">이력</button>
                    <button
  ...
  <!-- 편입 모달 -->
  <div v-if="showAbsorbModal" class="modal-overlay" @click.self="showAbsorbModal = false">
    <div class="modal modal-sm">
      <div class="modal-header">
        <h3>소과제로 편입</h3>
        <button class="modal-close" @click="showAbsorbModal = false">✕</button>
      </div>
      <div class="modal-body">
